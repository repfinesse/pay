<?php
include_once  $_SERVER['DOCUMENT_ROOT'] .'/data/data-functions.php';
...
class Webhook
{
    ...
    public function checkout_session_completed(){
        $this->log(); 
        echo "yo";
         $type    = $this->event->data->object->display_items[0]->type;
         $id       = $this->event->data->object->client_reference_id ;
         $gross = $this->event->data->object->display_items[0]->amount / 100;
        $sid =  $this->event->data->object->customer;
        if($_SESSION['sub']){
            $arr = explode("_", $id);
            print_r($arr);
            $plan = $arr[0];
            ...
            echo $client->sub("stripe", $gross, $plan, $sid);
        }else {
            $uses    = $this->event->data->object->display_items[0]->quantity;
            $client = new Client($id);
        echo $client->topup($uses);
        }
